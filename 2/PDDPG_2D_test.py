...
class ANet(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        self.convlstm = convlstm.ConvLSTM(input_dim = 1, hidden_dim = 1, kernel_size = (3,3), num_layers = 5, batch_first=True)
        self.fc1 = nn.Linear(2000, a_dim)
    ...
    def forward(self, s):
        # print(s.shape)
        xx = torch.zeros((1, 10, 1, 1, 2000))
        if use_gpu:
            xx = xx.cuda()
        xx[:,:,:,:,:1910] = s
        xx = xx.view(1, 10, 1, 40, 50)
        # print('xx is')
        # print(xx)
        # print('xx shape is')
        # print(xx.shape)
...
class PDDPG(object):
    def __init__(self, a_dim, s_dim):
        ...
        self.Actor_eval.load_state_dict(torch.load('result/0514/0514_0027/Actor_eval.pkl'))
        self.Actor_eval.eval()
        self.Actor_target.load_state_dict(torch.load('result/0514/0514_0027/Actor_target.pkl'))
        self.Actor_target.eval()
        self.Critic_eval.load_state_dict(torch.load('result/0514/0514_0027/Critic_eval.pkl'))
        self.Critic_eval.eval()
        self.Critic_target.load_state_dict(torch.load('result/0514/0514_0027/Critic_target.pkl'))
    ...
    def choose_action_1(self, state, var):
        if use_gpu:
            state = state.cuda()        
        ss = torch.unsqueeze(torch.FloatTensor(state), 0)
    ...
    def choose_action_2(self, state, EPSILON):
        ss = torch.FloatTensor(state)
        if use_gpu:
            ss = ss.cuda()  
        action = self.Actor_eval(ss)
    ...
    def learn(self):
        ...
        # if use_gpu:
        #     bt = bt.cuda()
        bs = torch.FloatTensor(bt[0])
        ba = torch.FloatTensor(bt[1])
        br = torch.FloatTensor(bt[2])
        bs_ = torch.FloatTensor(bt[3])
