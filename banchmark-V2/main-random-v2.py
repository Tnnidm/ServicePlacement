...
import output_PDDPG_2D1
import PDDPG_2D
import control_group.RandomPolicy
import config
...
'''
# dqn = dqn.DQN()
Agent = control_group.RandomPolicy.Random(a_dim)

# ddpg_h = PDDPG_2D.PDDPG(a_dim, s_dim)
# ddpg_w = PDDPG_2D.PDDPG(a_dim, s_dim)
# ddpg = ddpg.DDPG(a_dim, s_dim)
# retarder = Queue()
sys_per = output_PDDPG_2D1.SystemPerformance(a_dim)
line_fig = output_PDDPG_2D1.LineFigures(a_dim)
...
for Date in range(1011, 1013):
    ...
    for t in range(1, SLOTNUM):
        ...
        request_num_writer.writerow([Date, t, env.car_request_times, 0])

        s_ = mixed_loadrate
        disconnect_rate, Avg_Delay, Delay_Outage_Rate = output_PDDPG_2D1.Calculate_disconnect_outofdelay(env)
        r = output_PDDPG_2D1.Calculate_Reward_new(a, loadrate, disconnect_rate, Delay_Outage_Rate, env)
