...
class Env:
    ...
    def update(self, a, a_last, i_p, t):
        ...
        for key in self.dict_existing_car:
            self.car_request_times += self.dict_existing_car[key].update(t, self.dict_L1_INDE_cell, self.rho, self.list_INDE_object, self.INDE_cell)
    ...
    def Draw_L1_Car_Picture(self, i_p, t, PictureType):
        ...
        if PictureType != -1:
            ...
            plt.ylim(0, 10000)
            if PictureType == 0:
                plt.savefig(self.result_path+'/'+'result_images/'+str(self.Date)+'_'+str(i_p)+'_'+str(t)+'.png', dpi=600)
            elif PictureType == 1:
                plt.show()
                plt.savefig(self.result_path+'/'+'result_images/'+str(self.Date)+'_'+str(i_p)+'_'+str(t)+'.png', dpi=600)
                plt.pause(0.05)
