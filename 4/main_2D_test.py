...
filelist=GetPyFile(sourcefolder+'/4')
for file in filelist:
    old_pos = sourcefolder+'/4/'+file
...
request_num = open(path+'/request_num.csv', 'w', encoding='utf-8', newline='')
request_num_writer = csv.writer(request_num)
request_num_writer.writerow(['Date', 'Time', 'Car Request Times', 'Workload Update Times'])
...
state = np.zeros((1, LSTM_INPUT_SLOT_NUM, 1, 1, 1910))
state_ = np.zeros((1, LSTM_INPUT_SLOT_NUM, 1, 1, 1910))


for Date in range(1012, 1013):
    print('Date = '+str(Date))
    ...
    a_last = a
    s = np.transpose(s)
    state_[0,0:LSTM_INPUT_SLOT_NUM-1,0] = state_[0,1:LSTM_INPUT_SLOT_NUM,0]
    state[0,LSTM_INPUT_SLOT_NUM-1,0] = s
    # print(state)
    s_ = np.transpose(s_)
    ...
    for t in range(1, SLOTNUM):
        ...
        aa = ddpg_w.choose_action_3(state, EPSILON)
        a_t = copy.deepcopy(aa).numpy()
        a = aa[0,:1910].numpy() 
        ...
        request_num_writer.writerow([Date, t, env.car_request_times, a_dim])

        s_ = mixed_loadrate
        disconnect_rate, Avg_Delay, Delay_Outage_Rate = output_PDDPG_2D.Calculate_disconnect_outofdelay(env)
        r = output_PDDPG_2D.Calculate_Reward(a, loadrate, disconnect_rate, Delay_Outage_Rate)
        ...
        state_[0,0:LSTM_INPUT_SLOT_NUM-1,0] = state_[0,1:LSTM_INPUT_SLOT_NUM,0]
        state_[0,LSTM_INPUT_SLOT_NUM-1,0] = s_
        # print(state_)
        ...
        s = s_
        state[0,0:LSTM_INPUT_SLOT_NUM-1,0] = state[0,1:LSTM_INPUT_SLOT_NUM,0]
        state[0,LSTM_INPUT_SLOT_NUM-1,0] = s
        # print(state)
        # ep_r += r
        a_last = a
...
#         request_num_writer.writerow([Date, t, env.car_request_times, np.sum(a)])
