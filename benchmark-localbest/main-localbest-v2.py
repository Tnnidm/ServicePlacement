...
# import PDDPG_2D
import control_group.Localbest
import config
...
'''
# dqn = dqn.DQN()
Agent = control_group.Localbest.Localbest(a_dim)
...
for Date in range(1008, 1032):
    ...
    aa, loadrate, mixed_loadrate = env.update(a, a_last, 0, 0)
    s = mixed_loadrate
    a = aa[0,:] 
    ...
    for t in range(1, SLOTNUM):
        ...
        aa, loadrate, mixed_loadrate = env.update(a, a_last, i_episode, t+(Date-1001)*SLOTNUM)
        ...
        request_num_writer.writerow([Date, t, env.car_request_times, count])
