...
class Car:
    ...
    def update(self, t, dict_L1_INDE_cell, rho, list_INDE_object, INDE_cell, cellnum_of_IDNE, list_open_L1_INDE):
        ...
        # reset
        self.INDE = -1
        self.update_INDE(dict_L1_INDE_cell, rho, list_INDE_object, INDE_cell, cellnum_of_IDNE, list_open_L1_INDE)
        if self.INDE != -1:
            list_INDE_object[self.INDE].L1_connect_Car(self.id)
            if self.INDE == last_INDE:
                return 0
        else:
    ...
    def update_INDE(self, dict_L1_INDE_cell, rho, list_INDE_object, INDE_cell, cellnum_of_IDNE, list_open_L1_INDE):
        x = int(self.pos[0]/1000)
        y = int(self.pos[1]/1000)
        
        cell_number = 10*x+y
        self.find_available_INDE(cell_number, dict_L1_INDE_cell, list_INDE_object, cellnum_of_IDNE, list_open_L1_INDE)

        self.calculate_delay(self.INDE, rho, list_INDE_object)
        # self.find_closest_INDE(list_INDE_object, INDE_cell)


    def find_available_INDE(self, cell_number, dict_L1_INDE_cell, list_INDE_object, cellnum_of_IDNE, list_open_L1_INDE):
        ...
        for i in range(len(cellnum_of_IDNE[cell_number])):
            if list_INDE_object[((cellnum_of_IDNE[cell_number])[i])].L1_report_loadrate()<1 and list_INDE_object[((cellnum_of_IDNE[cell_number])[i])].L1_state==1:
                self.INDE = (cellnum_of_IDNE[cell_number])[i]
                pos0 = list_INDE_object[((cellnum_of_IDNE[cell_number])[i])].pos[0]
                pos1 = list_INDE_object[((cellnum_of_IDNE[cell_number])[i])].pos[1]
                self.distance = np.sqrt(np.square(self.pos[0]-pos0)+np.square(self.pos[1]-pos1))
                break
            elif list_INDE_object[((cellnum_of_IDNE[cell_number])[i])].L1_state==0:
                list_INDE_object[((cellnum_of_IDNE[cell_number])[i])].L1_open(list_open_L1_INDE, dict_L1_INDE_cell)
                self.INDE = (cellnum_of_IDNE[cell_number])[i]
                pos0 = list_INDE_object[((cellnum_of_IDNE[cell_number])[i])].pos[0]
                pos1 = list_INDE_object[((cellnum_of_IDNE[cell_number])[i])].pos[1]
                self.distance = np.sqrt(np.square(self.pos[0]-pos0)+np.square(self.pos[1]-pos1))                
...
class Env:
    ...
    def __init__(self, Date, path):
        ...
        self.cellnum_of_IDNE = {}
        for i in range(100):
            self.cellnum_of_IDNE.update({i:[]})
        for i in range(len(self.INDE_pos)):
            cell_num = 10*int(self.INDE_pos[i][0]/1000)+int(self.INDE_pos[i][1]/1000)
            self.cellnum_of_IDNE[cell_num].append(i)
        # for i in range(100):
        #     random.shuffle(self.cellnum_of_IDNE[i])
        ...
        self.car_request_times = 0
        self.workload_update_times = 0
    ...
    def update(self, a, a_last, i_p, t):
        ...
        for key in self.dict_existing_car:
            self.car_request_times += self.dict_existing_car[key].update(t, self.dict_L1_INDE_cell, self.rho, self.list_INDE_object, self.INDE_cell, self.cellnum_of_IDNE, self.list_open_L1_INDE)
        ...
        aa = np.zeros((1, a_dim))
        for i in range(a_dim):
            if self.load_rate[i,0]>0:
                aa[0,i] = 1
            elif self.list_INDE_object[i].L1_state == 1:
                self.list_INDE_object[i].L1_close(self.list_open_L1_INDE, self.dict_L1_INDE_cell, self.dict_existing_car)
        
        return aa, self.load_rate, self.mixed_load_rate
    ...
    def GetCarPos(self):
        CarPos = np.zeros((len(self.dict_existing_car),2))
        count = 0
        for key in self.dict_existing_car.keys():
