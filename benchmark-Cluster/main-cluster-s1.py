...
import PDDPG_2D
import control_group.ClusterPolicy
import config
...
'''
# dqn = dqn.DQN()
Agent = control_group.ClusterPolicy.Cluster(a_dim)
...
for Date in range(1012, 1013):
    ...
    for t in range(1, SLOTNUM):
        ...
        # aa = Agent.choose_action(env)
        aa = np.zeros((1,a_dim))
        a_t = copy.deepcopy(aa)
        a = aa[0,:]
        request_num_writer.writerow([Date, t, 2*(env.Print_System_Performance())[0], len(a)])
        for i in range(a_dim):
            if a[i] >= 0.5:
                a[i] = 1
            else:
...
for Date in range(1012, 1013):
    ...
    for t in range(1, SLOTNUM):
        ...
        loadrate, mixed_loadrate = env.update(a, a_last, i_episode, t+(Date-1001)*SLOTNUM)
        # loadrate, mixed_loadrate = env.update_temp(a, a, i_episode, t+(Date-1001)*SLOTNUM)
        s_ = mixed_loadrate
        disconnect_rate, Avg_Delay, Delay_Outage_Rate = output_PDDPG_2D.Calculate_disconnect_outofdelay(env)
        r = output_PDDPG_2D.Calculate_Reward_new(a, loadrate, disconnect_rate, Delay_Outage_Rate, env)
