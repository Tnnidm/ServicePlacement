...
class ANet(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        # self.convlstm = convlstm.ConvLSTM(input_dim = 1, hidden_dim = 1, kernel_size = (3,3), num_layers = CONVLSTM_LAYERS, batch_first=True)
        self.fcs1 = nn.Linear(s_dim, 100)
        self.fcs1.weight.data.normal_(0.1, 0.02)
        self.fcs2 = nn.Linear(100, 100)
        self.fcs2.weight.data.normal_(0.1, 0.02)
        self.fcs3 = nn.Linear(100, 100)
        self.fcs3.weight.data.normal_(0.1, 0.02)
        self.fcs4 = nn.Linear(100, 1000)
        self.fcs4.weight.data.normal_(0.1, 0.02)

        self.fc1 = nn.Linear(1000, 2*a_dim)
...
class PDDPG(object):
    ...
    def choose_action_3(self, state, EPSILON):
        ...
        # print(any(s[:,0,100]))
        for i in range(self.a_dim):
            if any(s[:,0,i]) == False and action[0,i] > 0.5 and np.random.uniform() <= EPSILON:
            # if s[0,i] == 0 and action[0,i] > 0.5:
                action[0,i] = 0
