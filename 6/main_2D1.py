...
ddpg_w = PDDPG_2D1.PDDPG(a_dim, s_dim)
...
mixed_loadrate_series = np.zeros((1, 10, 1, 1, a_dim))
...
for Date in range(1008, 1012):
    ...
    for i in range(1910):
        s[0, int(INDE_utilization_mapping[i])] += mixed_loadrate[i,0]
    ...
    mixed_loadrate_series[0,0:9,0] = mixed_loadrate_series[0,1:10,0]
    mixed_loadrate_series[0,9,0] = mixed_loadrate    
    ...
    for t in range(1, SLOTNUM):
        ...
        if Date == 1008:
            ...
            aa = ddpg_w.choose_action_2(state, mixed_loadrate_series, env, a_last, EPSILON)
